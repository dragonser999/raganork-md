...
async function lyricsSearch(query) {
  const { data } = await axios.get(
    "https://api.nexray.eu.cc/search/lyrics",
    { params: { q: query } }
  );
  if (!data || data.status !== true || !data.result?.lyrics) {
    throw new Error("Lyrics API returned no result");
  }
  return data;
}
...
Module(
  ...
  async (message, match) => {
    ...
    if (!query) {
      return await message.sendReply(
        "_Please provide a song name!_\n_Example: .lyrics jhol_"
      );
    }

    try {
      const searchMsg = await message.sendReply("_🔎 Searching lyrics..._");
      const data = await lyricsSearch(query);
      const r = data.result;
      const l = r.lyrics;

      const header = `_*${l.track_name || r.title}*_\n_by ${
        l.artist_name || r.artist
      }_`;
      const lyricsText = l.plain_lyrics || "_Lyrics not available for this track._";

      if (r.thumbnail && r.thumbnail !== "-") {
        try {
          const thumbResponse = await axios.get(r.thumbnail, {
            responseType: "arraybuffer",
          });
          const thumbnailBuffer = Buffer.from(thumbResponse.data);
          await message.edit("_Found! Sending..._", message.jid, searchMsg.key);
          await message.sendReply(thumbnailBuffer, "image", {
            caption: header,
          });
          await message.sendReply(lyricsText);
          return;
        } catch (e) {
          // if thumbnail fetch fails, fall back to plain text below
        }
      ...
      await message.edit(`${header}\n\n${lyricsText}`, message.jid, searchMsg.key);
    } catch (error) {
      console.error("Lyrics search error:", error);
      await message.sendReply(
        "_Lyrics not found. Please try again with a different query._"
      );
    }
  }
);
